...
void equi_linear_baseline(const float* x,
                          const float* weight,
                          const float* bias,
                          float* out,
                          size_t batch,
        ...
        for (size_t oc = 0; oc < out_channels; ++oc) {
            for (size_t d = 0; d < 16; ++d) {
                ...
                for (size_t ic = 0; ic < in_channels; ++ic) {
                    for (size_t s = 0; s < 16; ++s) {
                        sum +=
                            kernel[((oc * in_channels + ic) * 16 + d) * 16 + s] * x[b * in_channels * 16 + ic * 16 + s];
                    }
                }
...
// 6-loop version
// void equi_linear_baseline_fused(const float *x, const float *weight, const
// float *bias, float *out,
//                                 size_t batch, size_t in_channels, size_t
//                                 out_channels, bool normalize_basis)
// {
//     static const LocalEquiLinearBasis EQUI_BASIS;
//     const float (*basis)[16][16] = normalize_basis ? EQUI_BASIS.norm_basis :
//     EQUI_BASIS.unnorm_basis;
//
//     for (size_t b = 0; b < batch; ++b)
//     {
//         for (size_t oc = 0; oc < out_channels; ++oc)
...
//                         for (size_t s = 0; s < 16; ++s)
//                         {
//                             // opt L1 : wrap this in if (basis[w][d][s] !=
//                             0.0f)
//                             // opt L2 : change loop order to d -> w -> s ->
//                             check basis[w][d][s] != 0.0f -> ic
//                             //          so ic becomes innermost loop and we
//                             avoid all those nops sum += weight[(oc *
//                             in_channels + ic) * 9 + w] * basis[w][d][s] * x[b
//                             * in_channels * 16 + ic * 16 + s];
//                         }
//                     }
//                 }
//                 out[b * out_channels * 16 + oc * 16 + d] = sum;
